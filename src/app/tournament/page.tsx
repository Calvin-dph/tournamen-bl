    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a2332] via-[#2d3748] to-[#1a2332]">
      <div className="container mx-auto max-w-6xl py-10 px-4">
        ...
        <div className="flex flex-col gap-4 mb-6 md:mb-8">
          ...
          <div className="text-center">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#f5f7fa] mb-2">Active Tournaments</h1>
            <p className="text-[#d4af37] text-base md:text-lg">TI Billiard Cup 2025</p>
          </div>
        </div>
          ...
          <div className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
            {tournaments.map((tournament) => (
              <Link key={tournament.id} href={`/tournament/${tournament.id}`}>
                <Card className="bg-[#2d3748]/60 border-[#d4af37]/30 hover:border-[#d4af37]/60 transition-all hover:scale-105 hover:shadow-2xl hover:shadow-[#d4af37]/15 cursor-pointer h-full">
                  <CardHeader className="space-y-3 pb-3">
                    <div className="flex items-start justify-between gap-3">
                      <CardTitle className="text-lg sm:text-xl font-bold text-[#f5f7fa] leading-tight line-clamp-2 flex-1">
                        {tournament.name}
                      </CardTitle>
                      <Badge className={`text-xs font-medium flex-shrink-0 ${statusStyles[tournament.status] || ''}`}>
